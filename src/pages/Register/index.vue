<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>登录密码:</label>
        <input type="password" placeholder="请输入密码" v-model="password" name="password" v-validate="{ required: true, regex: /^[0-9A-Za-z]{6,20}$/ }" :class="{ invalid: errors.has('password') }" />
    ...
    async userRegister() {
      ...
      if (success) {
        try {
          ...
          await this.$store.dispatch('userRegister', { phone, password, password1, code })
          // 跳转到登录页
